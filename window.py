...
from dovea import application, color, input_handler
from dovea.scene import instance as scene  # for toggling collider visibility
...
class Window(WindowProperties):
    ...
    def _ready(self, title, icon, borderless, fullscreen, size, forced_aspect_ratio, position, vsync, editor_ui_enabled, window_type, render_mode):
        loadPrcFileData('', 'window-title dovea')
    ...
    def make_editor_gui(self):     # called by main after setting up camera and application.development_mode
        ...
        from dovea import Button, ButtonList, Entity, Func, Text, Tooltip, camera
        ...
        self.cog_menu = ButtonList({
            # 'Build' : Func(print, ' '),
            'API Reference' : Func(webbrowser.open, 'https://www.doveaengine.org/api_reference.html'),
            # 'Asset Store' : Func(webbrowser.open, 'https://itch.io/tools/tag-dovea'),
            'ursfx (Sound Effect Maker)' : lambda: exec('from dovea.prefabs import ursfx; ursfx.open_gui()'),
        ...
        info_text ='''This menu is not enabled in builds. To see how the app will look like in builds, do dovea(development_mode=False), which will disable all editor ui and start the app in fullscreen. To disable only this menu, do window.cog_menu.enabled = False'''
    ...
    def update_aspect_ratio(self):
        ...
        from dovea import camera, window
    ...
    def toggle_editor_camera(self):
        from dovea import EditorCamera, Text, camera, color
...
if __name__ == '__main__':
    ...
    # app = dovea(borderless=True, forced_aspect_ratio=16/9)
    app = dovea(
        title='dovea',
        # borderless=False,
        # borderless=True,
