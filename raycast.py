...
from dovea import doveamath, color
...
def raycast(origin, direction:Vec3=(0,0,1), distance=9999, traverse_target:Entity=scene, ignore:list=None, debug=False, color=color.white):
    ...
    entries = [        # filter out ignored entities
        ...
        and doveamath.distance(_raycaster.world_position, e.get_surface_point(builtins.render)) <= distance
        ]
    ...
    hit_info.distance = doveamath.distance(_raycaster.world_position, hit_info.world_point)
...
if __name__ == '__main__':
    from dovea import *
    from dovea import dovea, Entity, held_keys, time, duplicate, camera, EditorCamera
    app = dovea()
