...
from dovea import string_utilities
...
base = None             # this will be set once the dovea() is created
...
def load_settings(path=asset_folder / 'dovea_settings.py'):
    if path.exists():
        with open(path) as f:
            try:
                d = dict(locals(), **globals())
                exec(f.read(), d, d)
                # exec('from dovea import *\n' + f.read())
                string_utilities.print_info('loaded settings from dovea_settings.py successfully')
            except Exception as e:
                string_utilities.print_warning('warning: dovea_settings.py error:', e)
