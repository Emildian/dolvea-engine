...
from dovea.mesh import Mesh
from dovea import application, color
from time import perf_counter
from dovea.string_utilities import print_info, print_warning
from dovea.vec3 import Vec3
import panda3d.core as p3d
import gltf
import builtins
from dovea.sequence import Func
...
def load_model(name, folder=None, file_types=('.bam', '.doveamesh', '.obj', '.glb', '.gltf', '.blend'), use_deepcopy=False, gltf_no_srgb=Func(getattr, application, 'gltf_no_srgb')):
            ...
            for file_path in folder.glob(f'**/{name}{filetype}'):
                ...
                if filetype == '.doveamesh':
                    try:
                        ...
                        raise Exception('invalid doveamesh file:', file_path, e)


                if filetype == '.obj':
                    # print('found obj', file_path)
                    m = obj_to_doveamesh(folder=folder, name=name, return_mesh=True)
                    m.path = file_path
                    m.name = name
                    imported_meshes[name] = m
                    if not use_deepcopy:
                    ...
                    if blend_to_obj(file_path):
                        m = obj_to_doveamesh(folder=folder, name=name, return_mesh=True)
                        if not m:
                            raise ValueError('obj_to_doveamesh failed to convert:', file_path)
                        m.path = file_path
                        m.name = name
                        imported_meshes[name] = m
                        if not use_deepcopy:
...
def load_blender_scene(name, folder:Path=Func(getattr, application, 'asset_folder'), reload=False, skip_hidden=True, models_only=False, uvs=True, vertex_colors=True, normals=True, triangulate=True, decimals=4):
    ...
    out_file_path = scenes_folder / f'{name}.dovea_blender_scene'
    ...
    if reload or not out_file_path.exists():
        ...
        args = [
            ...
            '--background',
            '--python',
            application.internal_scripts_folder / '_blender_scene_to_dovea.py',
            out_file_path,
...
def obj_to_doveamesh(folder=Func(getattr, application, 'models_compressed_folder'), out_folder=Func(getattr, application, 'models_compressed_folder'), name='*', return_mesh=True, save_to_file=False, delete_obj=False):
    if callable(folder):
        folder = folder()
    if callable(out_folder):
        out_folder = out_folder()
    ...
    for file_path in folder.glob(f'**/{name}.obj'):
        ...
        out_path = (out_folder / file_path.stem).with_suffix('.doveamesh')
        # with open(out_path, 'w') as file:
        #     file.write(meshstring)
        mesh.save(folder=out_folder, name=f'{file_path.stem}.doveamesh')

        if delete_obj:
            os.remove((out_folder / file_path.stem).with_suffix('.obj'))

        print_info('saved doveamesh to:', out_path)
    ...
    for f in files:
        if f.endswith('.blend'):
            ...
            for o in blend.list('Object'):
                ...
                file_name = ''.join([f.split('.')[0], '.doveamesh'])
                if number_of_objects > 1:
                    file_name = ''.join([f.split('.')[0], '_', object_name, '.doveamesh'])
...
def dovea_mesh_to_obj(mesh, name='', out_path=Func(getattr, application, 'models_compressed_folder'), max_decimals=5, flip_faces=True):
    if callable(out_path):
        out_path = out_path()

    from dovea.string_utilities import camel_to_snake
...
def compress_internal():
    for blend_file in application.internal_models_folder.glob('*.blend'):
        ...
        obj_to_doveamesh(application.internal_models_compressed_folder, application.internal_models_compressed_folder, return_mesh=False, save_to_file=True, delete_obj=True)


if __name__ == '__main__':
    compress_internal()
    from dovea import *
    from dovea import dovea, Entity, EditorCamera, Sky
    app = dovea()
    # print('imported_meshes:\n', imported_meshes)
    # Entity(model='quad').model.save('quad.bam')
    m = obj_to_doveamesh(folder=application.asset_folder.parent / 'samples', name='procedural_rock_0', save_to_file=False, delete_obj=False)
    print(m.serialize())
    # Entity(model=m)
    # EditorCamera()
    ...
    # Entity(model='c1a0')
    # from dovea.shaders import lit_with_shadows_shader
    ...
    # from dovea.prefabs.first_person_controller import FirstPersonController
    ...
    # dovea_mesh_to_obj(e.model, name='quad_export_test')
